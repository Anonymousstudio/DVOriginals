...
const cartRoutes = async (fastify) => {
    ...
    fastify.get('/', {
        ...
        try {
            ...
            let cart = await index_1.prisma.cart.findFirst({
                where: { userId },
                orderBy: { updatedAt: 'desc' }
            });
            if (!cart) {
                cart = await index_1.prisma.cart.create({
                    data: {
                        userId,
                        items: []
                    }
    ...
    fastify.put('/', {
        ...
        try {
            ...
            const validProducts = await index_1.prisma.product.findMany({
                where: {
                    id: { in: productIds },
                    isActive: true
                },
            ...
            const cart = await index_1.prisma.cart.upsert({
                where: {
    ...
    fastify.delete('/', {
        ...
        try {
            ...
            await index_1.prisma.cart.updateMany({
                where: { userId },
                data: { items: [] }
            });
            return {
